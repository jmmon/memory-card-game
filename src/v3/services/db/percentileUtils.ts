...
export const calculatePercentilesWhileMaintainingOrder = (
  ...
  // console.log({allScores, allScoreCounts});
  const { scoresByDeckSize, orderedListOfScoreIds } = allScores.reduce(
    (
      ...
      if (accum.scoresByDeckSize[curScore.deckSize]) {
        accum.scoresByDeckSize[curScore.deckSize].push(curScore);
      } else {
        accum.scoresByDeckSize[curScore.deckSize] = [curScore];
      }

      // maintain ID order
      accum.orderedListOfScoreIds.push(curScore.id);
      return accum;
    },
  ...
  // calculate percentiles by deck size
  for (let i = 0; i < allScoreCounts.length; i++) {
    const {
      deckSize,
      totalScores: totalScoresForDeckSize,
    ...
    // => we get all scorecounts but might not get scores for each of those decksizes
    if (scoresByDeckSize[deckSize] === undefined) {
      continue;
    }
    ...
    scoresByDeckSize[deckSize].forEach((score) => {
      ...
      allScoresWithPercentilesByScoreId[score.id] = buildScoreWithPercentiles(
        ...
        totalScoresForDeckSize,
      );
    });
  }

  // reorder by original sorted order
